...
public class MainController implements Initializable { // fazendo referencia a interface Initializable para poder usar o
		...
		this.tableContatos.getSelectionModel().selectedItemProperty()
				.addListener((observador, contatoAntigo, contatoNovo) -> {
					if (contatoNovo != null) {// se selecionou algum contato
						...
						txfId.setText(String.valueOf(contatoNovo.getId()));
						this.contatoSelecionado = contatoNovo; // passa informação de qual contato está selecionado para
																// poder usar nos metodos alterar e excluir
					}
				});
	...
	public void botaoExcluir_Action() {
		...
		Optional<ButtonType> resultadoConfirmacao = confirmação.showAndWait(); // envia a mensagem, e utiliza optional
																				// pq showAndWait retorna um
																				// optional<ButtonType>

		if (resultadoConfirmacao.isPresent() && resultadoConfirmacao.get() == ButtonType.OK) { // verifica se obteu um
																								// resultado no click do
																								// usuario e se o
																								// resultado foi "Ok"
			...
			try {
				...
				alerta.setContentText("Erro ao inserir um contato: " + e.getMessage());
				alerta.showAndWait();
			}
			carregarTabelaContatos();
	...
	public void botaoCancelar_Action() {
		habilitarEdicaoAgenda(false);
		this.tableContatos.getSelectionModel().selectFirst(); // retornar a seleção para o primeiro elemento da
																// tableView
	}

	public void botaoSalvar_Acition() {
		try {
			AgendaRepositorio<Contato> repositorioContato = new ContatoRepositorioJDBC(); // instancia
																							// ContatoRepositorio para
																							// obter os metodos de
																							// alterações e
																							// inclusão
			...
			if (this.ehInserir) {// se inserir tiver true, é inserir, senão, é atualizar
				...
				repositorioContato.atualizar(contato, Integer.parseInt(this.txfId.getText()));
			}
			habilitarEdicaoAgenda(false);
	...
	private void carregarTabelaContatos() {
		try {
			// cria uma nova instancia de ContatoReposito para poder manipular
			AgendaRepositorio<Contato> repositorioContato = new ContatoRepositorioJDBC();
			// obtem os contatos que são retornados de ContatoRepositorio
			...
			ObservableList<Contato> contatosObserverList = FXCollections.observableArrayList(contatos);// inserir um
			...
			mensagem.setHeaderText("Erro do banco de dados");
			mensagem.setContentText("Ouve um erro ao obter a lista de contatos " + e.getMessage());
		}
	}
